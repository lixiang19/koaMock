...
// @version      1.0.3
// @description 油猴脚本，需要chrome浏览器安装油猴后再把这个脚本安装到油猴内
// @author      lixiang
...
@keyframes myRotate{
  ...
  function createMock() {
    ...
    elementArr.forEach(ele => {
      ...
      ele.querySelector('.http_method').prepend(button)
      const url = ele.querySelector('.path').children[0].innerHTML
      const method = ele.querySelector('.http_method').children[3].innerHTML
      const note = ele.querySelector('.markdown').children[0].innerHTML
      const obj = {
        url,
        method,
  ...
  function generateMockString(description) {
    return `Random.string(3,5), //${description}`
  }
